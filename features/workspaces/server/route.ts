...
const app = new Hono()
  ...
  const fileViewBuffer = await storage.getFileView(IMAGES_BUCKET_ID, file.$id);
  // Convert ArrayBuffer to a base64 string URL (or handle as needed)
  uploadedImageUrl = `data:image/png;base64,${Buffer.from(fileViewBuffer).toString("base64")}`;
}
